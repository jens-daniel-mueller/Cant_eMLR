...
all_lm_wide <- all_lm_wide %>% 
  mutate(JGOFS_GO = coeff_JGOFS_WOCE - coeff_GO_SHIP,
         GO_new = coeff_GO_SHIP - coeff_new_era) %>% 
...
Cant_model_average <- Cant %>% 
  mutate(Cant = -Cant) %>%
  mutate(Cant = if_else(Cant < 0, 0, Cant)) %>% 
  group_by(lon, lat, depth, eras, basin) %>% 
  summarise(Cant_mean = mean(Cant)) %>% 
  ungroup()

Cant_model_average_section <- Cant_model_average %>% 
  group_by(lat, depth, eras, basin) %>% 
  summarise(Cant_zonal_mean = mean(Cant_mean, na.rm = TRUE)) %>% 
  ungroup()
...
```{r Cant_Atlantic_section_mean_surface}

Cant_model_average_section %>% 
  filter(depth <= 500,
         lat > -60.6) %>% 
  ggplot(aes(lat, depth, z = Cant_zonal_mean)) +
