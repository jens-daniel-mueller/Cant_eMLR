...
```{r control_plots_pre_merging_WOA18, fig.asp=0.6}
...
```{r interpolate_WOA18_vertically}
...
```{r joined_climatology_control_PO4}
map_climatology(predictors, "PO4")
```


```{r joined_climatology_control_tem}
map_climatology(predictors, "tem")
```


```{r WOA18_GLODAP_climatology_control_plots, fig.asp=0.6}
...
```{r predictors_gamma_mean_sections}
...
```{r Cant_sections_positive_mean_one_long}
