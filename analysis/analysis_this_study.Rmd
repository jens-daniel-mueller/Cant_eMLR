...
for (i_eras in unique(cant_gamma_maps$eras)) {
  for (i_gamma_slab in unique(cant_gamma_maps$gamma_slab)) {
    print(
      p_map_cant_pos_slab(
      cant_gamma_maps %>% filter(eras == i_eras,
...
for (i_eras in unique(cant_inv$eras)) {
  ...
  print(p_map_cant_pos_inv(
    cant_inv %>% filter(eras == i_eras),
    subtitle_text = paste("Eras:", i_eras)
  ))
...
print(
p_section_zonal_divergent_gamma_eras_basin(cant_predictor_zonal,
                          variable,
                          "gamma")
)
