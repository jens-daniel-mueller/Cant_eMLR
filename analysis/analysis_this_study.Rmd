...
Definition of color scale as applied in Gruber et al (2019).
...
```{r Cant_sections_positive_mean, fig.asp=1}
...
for (i_basin_AIP in unique(Cant_average_zonal$basin_AIP)) {
  for (i_eras in unique(Cant_average_zonal$eras)) {
...
#   coord_cartesian(expand = 0) +
#   guides(fill = guide_colorsteps(barheight = unit(10, "cm"))) +
#   facet_wrap(~ basin_AIP + eras, ncol = 1)
...
```{r Cant_gamma_slab_maps, fig.asp=2.5}
