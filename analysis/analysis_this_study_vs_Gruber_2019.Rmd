...
```{r cant_pos_delta_inventory_map}
...
```{r cant_pos_delta_zonal_section, fig.asp=1}
...
```{r cant_delta_inventory_map, fig.asp=1}
...
cant_inventory_wide %>% 
  ggplot() +
    geom_raster(data = landmask,
                aes(lon, lat), fill = "grey80") +
    geom_raster(aes(lon, lat, fill = cut(cant_inv_delta, breaks))) +
    coord_quickmap(expand = 0) +
    scale_fill_scico_d(palette = "vik", drop = FALSE) +
    theme(axis.title = element_blank())
...
```{r cant_mean_zonal_section, fig.asp=1}
...
```{r cant_mean_delta_zonal_section, fig.asp=1}
