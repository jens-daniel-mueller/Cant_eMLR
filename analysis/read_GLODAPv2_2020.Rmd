...
GLODAP_obs_grid_temp <- GLODAP %>% 
  count(lat, lon, era) %>% 
  mutate(cleaning_level = "below_150m")
...
```{r apply_lat_max_and_bottomdepth_min}
...
GLODAP_stats_temp <- GLODAP %>% 
  summarise(below_150m_65N_500m = n())
...
GLODAP_obs_grid_temp <- GLODAP %>% 
  count(lat, lon, era) %>% 
  mutate(cleaning_level = "below_150m_65N_500m")
...
GLODAP_obs_grid_temp <- GLODAP %>% 
  count(lat, lon, era) %>% 
  mutate(cleaning_level = "basinmask")
...
GLODAP_obs_grid_temp <- GLODAP %>% 
  count(lat, lon, era) %>% 
  mutate(cleaning_level = "Cstar_variables")
...
GLODAP_obs_grid %>%
  ggplot(aes(lon, lat, fill = cleaning_level)) +
  geom_raster(data = landmask %>% filter(region == "land"),
              aes(lon, lat), fill = "grey80") +
  geom_raster() +
