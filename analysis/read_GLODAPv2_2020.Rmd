...
Only samples with all relevant parameters determined were selected.
The following flagging criteria were defined:
...
GLODAP <- GLODAP %>% 
  mutate(Latitude = cut(latitude, seq(-90, 90, 1), seq(-89.5, 89.5, 1)),
         Latitude = as.numeric(as.character(Latitude)),
         Longitude = cut(longitude, seq(-180, 180, 1), seq(-179.5, 179.5, 1)),
...
GLODAP_map_era <- GLODAP %>% 
  group_by(era, Latitude, Longitude) %>% 
  tally() %>% 
  ungroup()
...
GLODAP_map_era %>%
  ggplot(aes(Longitude, Latitude, fill=log10(n)))+
  mapWorld+
  geom_tile()+
  scale_fill_viridis_c(option = "magma", direction = -1)+
