...
GLODAP <- GLODAP %>%
  ...
  mutate(era = "JGOFS-WOCE",
         ...
         era = if_else(year > GOSHIP, "new-era", era))
...
```{r coverage_maps_era_bin2d, fig.asp=1.5}

mapWorld <- borders("world", colour="gray60", fill="gray60")


GLODAP <- GLODAP %>% 
  mutate(era = factor(era, c("JGOFS-WOCE", "GO-SHIP", "new-era")))
