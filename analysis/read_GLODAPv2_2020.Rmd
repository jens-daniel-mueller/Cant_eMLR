...
Below, the data coverage is displayed for each step of the cleaning process. Currently, we only remove rows with missing values, but keep data with all flags.

Summary statistics were calculated during cleaning process.


Following flagging criteria were previously applied, but are not yet taken into account anymore:
...
The majority of rows is removed due to missing tco2 observations.
...
Restricting the qc flag to 1 would results in data gaps in the south Indian ocean and equatorial Pacific. Data for which secondary quality was not applied (qc flag 0) are therefore included.
...
Restricting the qc flag to 1 would results in data gaps in the equatorial Pacific. Data for which secondary quality was not applied (qc flag 0) are therefore included.
...
Rows with missing eMLR variables were removed, flagging was not considered
...
```{r grid_spatially_coarse, eval=FALSE}
