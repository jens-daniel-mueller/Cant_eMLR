...
Below, the data coverage is displayed for each step of the cleaning process. Currently, we remove all rows with missing values, and decide about flagging on a per variable basis.
...
We keep tco2 data with f flag = 0 (interpolated or calculated). 
...
We discard tco2 data with qc flag = 0 (no sec qc). 
...
talk ata for which secondary quality was not applied (qc flag 0) were removed.
