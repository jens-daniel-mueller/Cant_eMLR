...
```{r WOA18_pot_temperature_section, fig.asp=0.6}
...
For water masses with neutral densities below `r parameters$gamma_min`, only predictor fields with a water depth higher than `r parameters$depth_min`m were taken into account.
...
```{r read_WOA18_predictor_file_gamma}

rm(list = setdiff(ls(), c("landmask", "parameters", "section_global_coordinates")))
source(here::here("code", "plotting_functions.R"))
...
```{r temperature_sections_WOA18, fig.asp=0.6}
...
```{r salinity_sections_WOA18, fig.asp=0.6}
...
```{r neutral_density_sections_WOA18, fig.asp=0.6}
