...
Here, we use the objectively analyzed climatologies.
...
WOA_tem_tibble %>% 
  ...
  geom_raster() +
  geom_vline(xintercept = section_lon, col = "red") + 
  coord_quickmap(expand = 0) +
...
WOA_tem_tibble %>% 
  filter(lon == section_lon) %>% 
  ggplot(aes(lat, depth, z = t_an)) +
  geom_contour_filled(name = "t_an") +
  scale_y_reverse() +
